...
void benchmark(double zipf_theta) {
  ...
  std::vector<size_t> key_input;
  std::ifstream input_("/home/heejin/trace_insert_only.csv");
  if(input_.is_open()) {
    while(!input_.eof()) {
      char buf[255];
      size_t input_num;
      input_.getline(buf, 255);
      sscanf(buf, "%lu", &input_num);
      key_input.push_back(input_num);
    }

  }

  size_t input_size = key_input.size();
  size_t num_items = key_input.size();
  ...
  size_t num_operations = key_input.size();
...
*/
  ...
  for (uint16_t thread_id = 0; thread_id < num_threads; thread_id++) {
    ...
    task.value_array = (int*)malloc(sizeof(size_t) * max_num_operations_per_thread);
  ...
  for (size_t i = 0; i < num_items * 2; i++) {
    *(uint64_t*)(keys + i * key_length) = key_input[i%input_size];
    *(key_hashes + i) = hash(keys + i * key_length, key_length);
    *(key_parts + i) = processor.get_partition_id(*(key_hashes + i), false,\
